...
def getFacebookPageFeedData(page_id, access_token):
    ...
    node = "/%s/feed" % page_id 
    fields = "/?fields=from,message,comments"
    parameters = "&limit=1000&access_token=%s" % (access_token)
...
def scrapeFacebookPageFeedStatus(page_id, access_token):
    ...
    while has_next_page:
        ...
        for post in statuses['data']:
            total_posts+=1
            filepath=cwd+"/Posts_Pune/%s.txt"%total_posts
            file = open(filepath,'wb')
            if 'message' in post:
                post_message = post['message'].encode('ascii', 'ignore')
                file.write(post_message + "\n\n Comments : \n")
            if 'comments' in post:
                for comment in post['comments']['data']:
                    comment_message = comment['from']['name'].encode('ascii', 'ignore') + " : " + comment['message'].encode('ascii', 'ignore')
        ...
        if 'paging' in statuses.keys():
            print "\n\n\n"
            print "Accessing this page : %s " + str(statuses['paging']['next'])
    ...
    print "\nDone!\n%s Statuses Processed in %s" % \
        (total_posts, datetime.datetime.now() - scrape_starttime)
